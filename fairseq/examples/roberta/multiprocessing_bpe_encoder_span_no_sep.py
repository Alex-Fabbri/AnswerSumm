...
class MultiprocessingEncoder(object):
    ...
    def encode_lines(self, lines):
        ...
        for count, line_text in enumerate(lines):
            ...
            actual_line = actual_line.replace(u'\xa0', u' ')
            actual_line = actual_line.replace(u'\u3000', u' ')
            actual_line = actual_line.replace(" "*1345, " ")
            ...
            for span_sent in span_sents:
                ...
                span_sent = span_sent.replace(u'\xa0', u' ')
                span_sent = span_sent.replace(u'\u3000', u' ')
                ...
                if "grand master of the Grand Orange Lodge of Ireland, said" in span_sent:
                    span_sent += "\"â€ª"
                if span_sent.replace("\"â€ª", "") in actual_line:
                    span_sent = span_sent.replace("\"â€ª", "")
                
                # span_sent = span_sent.replace(u'\xa0', u' ')
                span_sent = span_sent.strip()
                # if "<S>" + span_sent in actual_line:
                #     span_sent = "<S>" + span_sent
                # else:
                #     for i in range(0, 100):
                #         if "<S>" + " " * i + span_sent in actual_line:
                #             span_sent ="<S>" + " " * i + span_sent
                #             break
                #     else:
                #         from fairseq import pdb;pdb.set_trace()
                # if span_sent + "<S>" in actual_line:
                #     span_sent = span_sent + "<S>"
                # else:
                #     for i in range(0, 100):
                #         if span_sent + " " * i + "<S>" in actual_line:
                #             span_sent = span_sent + " " * i + "<S>"
                #             break
                #     else:
                #         from fairseq import pdb;pdb.set_trace()
                ...
                try:
                    ...
                    # cur_str = self.decode([int(x) for x in tokens[start:end+1]])
                    # from fairseq import pdb;pdb.set_trace()
                    # print(cur_str, span_sent)
                    if end < start:
                        from fairseq import pdb;pdb.set_trace()
