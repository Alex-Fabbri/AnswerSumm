...
class MultiprocessingEncoder(object):
    ...
    def encode_lines(self, lines):
        ...
        for line_text in lines:
            ...
            span_sents = line_text.split("MYSEP")[1].split("SENT_SEP")
            all_spans = []
            for span_sent_overall in span_sents:
                tmp_spans = []
                # from fairseq import pdb;pdb.set_trace()
                cur_spans = span_sent_overall.split("INTERNAL_EXAMPLE")
                for span_sent in cur_spans:
                    ...
                    try:
                        ...
                        print(actual_line)
                # from fairseq import pdb;pdb.set_trace()
                tmp_spans.append(str(100000)) # -> 34464
                all_spans.extend(tmp_spans)
            
            enc_spans.append(" ".join(all_spans))
            enc_lines.append(" ".join(tokens))
        return ["PASS", enc_lines, enc_spans]
