...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False, print_ids=False, loss_index=-1, nll_only=False, span_only=False):
        ...
        for i, sample in enumerate(samples):
            ...
            # if print_ids:
            #     print(i, sample['id'])
            # sample = torch.tensor([26478, 74600, 87170])
            sample = self._prepare_sample(sample)
            # from fairseq import pdb;pdb.set_trace()
            # sentences = sample['net_input']['prev_input_tokens'][0]
            # out = self.model.encoder.dictionary.string(sentences)
            # cur_str = self.model.bpe.decode(out)
            # sentences = [self.model.bpe.decode() for s in sentences]
            if sample is None:
    ...
    def valid_step(self, sample, raise_oom=False, nll_only=False, index_loss=-1, span_only=True):
